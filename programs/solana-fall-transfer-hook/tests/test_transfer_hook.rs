...
use helpers::{
    ...
    build_transfer_with_token_mover_ix,
};
...
fn test_transfer_with_token_mover() {
    ...
    let transfer_ix = build_transfer_with_token_mover_ix(
        &source_ata,
        &dest_ata,
        &mint.pubkey(),
        &payer.pubkey(),
...
#[test]
fn test_transfer_with_token_mover_rate_limit_exceeded() {
    let (mut svm, payer, program_id) = setup();
    let mint = Keypair::new();
    ...
    // First transfer: exactly at the limit.
